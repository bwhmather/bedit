...
class Parser:
    ...
    def _match_env(self):
        text = self.remains()
        match = re.match("\\$(%s)" % self.SREG_ENV, text) or re.match(
            "\\${(%s)}" % self.SREG_ENV, text
        )
    ...
    def _match_placeholder(self):
        ...
        match = re.match(
            "\\${(%s)(:((\\\\\\}|[^}])+))?}" % self.SREG_ID, text
        ) or re.match("\\$(%s)" % self.SREG_ID, text)
    ...
    def _match_eval(self):
        ...
        groups = match.groups()
        (tabstop, depend) = (
            groups[0] and self._eval_options(groups[0][:-1])
        ) or (-1, [])
        self.position += len(match.group(0))
    ...
    def _match_regex(self):
        ...
        content = "((?:\\\\[/]|\\\\}|[^/}])+)"
        match = re.match(
            "\\${(?:(%s):)?\\s*(%s|\\$([A-Z_]+))?[/]%s[/]%s(?:[/]([a-zA-Z]*))?}"
            % (self.SREG_ID, self.SREG_ID, content, content),
            text,
        )
