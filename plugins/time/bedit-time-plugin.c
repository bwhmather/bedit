...
static void update_ui(BeditTimePlugin *plugin) {
    BeditView *view;
...
static void bedit_time_plugin_app_deactivate(BeditAppActivatable *activatable) {
    BeditTimePluginPrivate *priv;
...
static gchar *get_custom_format(BeditTimePlugin *plugin) {
    gchar *format;
...
static gchar *get_time(const gchar *format) {
    gchar *out;
    GDateTime *now;
...
static void create_formats_list(
    ...
    g_signal_connect(listview, "realize", G_CALLBACK(scroll_to_selected), NULL);
...
static void real_insert_time(GtkTextBuffer *buffer, const gchar *the_time) {
...
static void bedit_time_plugin_class_init(BeditTimePluginClass *klass) {
