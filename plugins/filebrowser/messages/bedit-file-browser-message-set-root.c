...
G_DEFINE_TYPE_EXTENDED (BeditFileBrowserMessageSetRoot,
                        bedit_file_browser_message_set_root,
                        GEDIT_TYPE_MESSAGE,
                        0,
                        G_ADD_PRIVATE (BeditFileBrowserMessageSetRoot))

static void
bedit_file_browser_message_set_root_finalize (GObject *obj)
{
	...
	G_OBJECT_CLASS (bedit_file_browser_message_set_root_parent_class)->finalize (obj);
}

static void
bedit_file_browser_message_set_root_get_property (GObject    *obj,
                                                  guint       prop_id,
                                                  GValue     *value,
                                                  GParamSpec *pspec)
{
...
static void
bedit_file_browser_message_set_root_set_property (GObject      *obj,
                                                  guint         prop_id,
                                                  GValue const *value,
                                                  GParamSpec   *pspec)
{
...
static void
bedit_file_browser_message_set_root_class_init (BeditFileBrowserMessageSetRootClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	object_class->finalize = bedit_file_browser_message_set_root_finalize;

	object_class->get_property = bedit_file_browser_message_set_root_get_property;
	object_class->set_property = bedit_file_browser_message_set_root_set_property;
...
static void
bedit_file_browser_message_set_root_init (BeditFileBrowserMessageSetRoot *message)
{
	message->priv = bedit_file_browser_message_set_root_get_instance_private (message);
}
