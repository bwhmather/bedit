...
G_DEFINE_TYPE_EXTENDED (BeditFileBrowserMessageIdLocation,
                        bedit_file_browser_message_id_location,
                        GEDIT_TYPE_MESSAGE,
                        0,
                        G_ADD_PRIVATE (BeditFileBrowserMessageIdLocation))

static void
bedit_file_browser_message_id_location_finalize (GObject *obj)
{
	...
	G_OBJECT_CLASS (bedit_file_browser_message_id_location_parent_class)->finalize (obj);
}

static void
bedit_file_browser_message_id_location_get_property (GObject    *obj,
                                                     guint       prop_id,
                                                     GValue     *value,
                                                     GParamSpec *pspec)
{
...
static void
bedit_file_browser_message_id_location_set_property (GObject      *obj,
                                                     guint         prop_id,
                                                     GValue const *value,
                                                     GParamSpec   *pspec)
{
...
static void
bedit_file_browser_message_id_location_class_init (BeditFileBrowserMessageIdLocationClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	object_class->finalize = bedit_file_browser_message_id_location_finalize;

	object_class->get_property = bedit_file_browser_message_id_location_get_property;
	object_class->set_property = bedit_file_browser_message_id_location_set_property;
...
static void
bedit_file_browser_message_id_location_init (BeditFileBrowserMessageIdLocation *message)
{
	message->priv = bedit_file_browser_message_id_location_get_instance_private (message);
}
