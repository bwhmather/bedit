...
G_DEFINE_TYPE_EXTENDED (BeditFileBrowserMessageAddFilter,
                        bedit_file_browser_message_add_filter,
                        GEDIT_TYPE_MESSAGE,
                        0,
                        G_ADD_PRIVATE (BeditFileBrowserMessageAddFilter))

static void
bedit_file_browser_message_add_filter_finalize (GObject *obj)
{
	...
	G_OBJECT_CLASS (bedit_file_browser_message_add_filter_parent_class)->finalize (obj);
}

static void
bedit_file_browser_message_add_filter_get_property (GObject    *obj,
                                                    guint       prop_id,
                                                    GValue     *value,
                                                    GParamSpec *pspec)
{
...
static void
bedit_file_browser_message_add_filter_set_property (GObject      *obj,
                                                    guint         prop_id,
                                                    GValue const *value,
                                                    GParamSpec   *pspec)
{
...
static void
bedit_file_browser_message_add_filter_class_init (BeditFileBrowserMessageAddFilterClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	object_class->finalize = bedit_file_browser_message_add_filter_finalize;

	object_class->get_property = bedit_file_browser_message_add_filter_get_property;
	object_class->set_property = bedit_file_browser_message_add_filter_set_property;
...
static void
bedit_file_browser_message_add_filter_init (BeditFileBrowserMessageAddFilter *message)
{
	message->priv = bedit_file_browser_message_add_filter_get_instance_private (message);
}
