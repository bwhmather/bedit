...
static GdkPixbuf *process_icon_pixbuf(
    GdkPixbuf *pixbuf, gchar const *name, gint size, GError *error) {
    GdkPixbuf *scale;
...
GdkPixbuf *bedit_file_browser_utils_pixbuf_from_theme(
    ...
    pixbuf = gtk_icon_theme_load_icon(
        gtk_icon_theme_get_default(), name, width, 0, &error);
...
GdkPixbuf *bedit_file_browser_utils_pixbuf_from_icon(
    ...
    info = gtk_icon_theme_lookup_by_gicon(
        theme, icon, width, GTK_ICON_LOOKUP_USE_BUILTIN);
...
GdkPixbuf *bedit_file_browser_utils_pixbuf_from_file(
    ...
    info =
        g_file_query_info(file, attribute, G_FILE_QUERY_INFO_NONE, NULL, NULL);
