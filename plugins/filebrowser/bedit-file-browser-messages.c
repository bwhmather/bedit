...
#define BUS_CONNECT(bus, name, data) bedit_message_bus_connect(bus, MESSAGE_OBJECT_PATH, #name, (BeditMessageCallback)  message_##name##_cb, data, NULL)
#define BUS_DISCONNECT(bus, name, data) bedit_message_bus_disconnect_by_func(bus, MESSAGE_OBJECT_PATH, #name, (BeditMessageCallback)  message_##name##_cb, data)
...
static WindowData *
window_data_new (BeditWindow            *window,
		 BeditFileBrowserWidget *widget)
{
...
static WindowData *
get_window_data (BeditWindow *window)
{
	return (WindowData *) (g_object_get_data (G_OBJECT (window), WINDOW_DATA_KEY));
}

static void
window_data_free (BeditWindow *window)
{
...
static FilterData *
filter_data_new (BeditWindow  *window,
		 BeditMessage *message)
{
	...
	g_hash_table_insert (wdata->filters,
			     bedit_message_type_identifier (bedit_message_get_object_path (message),
...
filter_data_free (FilterData *data)
{
	...
	identifier = bedit_message_type_identifier (bedit_message_get_object_path (data->message),
...
message_cache_data_new (BeditWindow  *window,
			BeditMessage *message)
{
...
static void
message_get_root_cb (BeditMessageBus *bus,
		     BeditMessage    *message,
		     WindowData      *data)
{
...
static void
message_set_root_cb (BeditMessageBus *bus,
		     BeditMessage    *message,
		     WindowData      *data)
{
	GFile *root;
	...
	if (virtual)
	{
		bedit_file_browser_widget_set_root_and_virtual_root (data->widget, root, virtual);
	}
	else
	{
...
static void
message_set_emblem_cb (BeditMessageBus *bus,
		       BeditMessage    *message,
		       WindowData      *data)
{
	gchar *id = NULL;
	...
	if (path != NULL)
	{
		...
		if (gtk_tree_model_get_iter (GTK_TREE_MODEL (store), &iter, path))
		{
			...
			bedit_file_browser_store_set_value (store,
			                                    &iter,
...
static void
message_set_markup_cb (BeditMessageBus *bus,
		       BeditMessage    *message,
		       WindowData      *data)
{
	gchar *id = NULL;
...
static gboolean
custom_message_filter_func (BeditFileBrowserWidget *widget,
			    BeditFileBrowserStore  *store,
			    GtkTreeIter            *iter,
			    FilterData             *data)
{
...
static void
message_add_filter_cb (BeditMessageBus *bus,
                       BeditMessage    *message,
                       BeditWindow     *window)
{
	...
	if (!bedit_message_type_check (message_type, "id", G_TYPE_STRING) ||
	    ...
	    !bedit_message_type_check (message_type, "is-directory", G_TYPE_BOOLEAN) ||
	...
	id = bedit_file_browser_widget_add_filter (data->widget,
...
static void
message_remove_filter_cb (BeditMessageBus *bus,
		          BeditMessage    *message,
		          WindowData      *data)
{
	gulong id = 0;
...
static void
message_extend_context_menu_cb (BeditMessageBus *bus,
				BeditMessage    *message,
				BeditWindow     *window)
{
	WindowData *data;
...
static void
message_up_cb (BeditMessageBus *bus,
	       BeditMessage    *message,
	       WindowData      *data)
{
	BeditFileBrowserStore *store = bedit_file_browser_widget_get_browser_store (data->widget);

	bedit_file_browser_store_set_virtual_root_up (store);
}

static void
message_history_back_cb (BeditMessageBus *bus,
		         BeditMessage    *message,
		         WindowData      *data)
{
...
static void
message_history_forward_cb (BeditMessageBus *bus,
		            BeditMessage    *message,
		            WindowData      *data)
{
...
static void
message_refresh_cb (BeditMessageBus *bus,
		    BeditMessage    *message,
		    WindowData      *data)
{
...
static void
message_set_show_hidden_cb (BeditMessageBus *bus,
		            BeditMessage    *message,
		            WindowData      *data)
{
...
static void
message_set_show_binary_cb (BeditMessageBus *bus,
		            BeditMessage    *message,
		            WindowData      *data)
{
	gboolean active = FALSE;
...
static void
message_show_bookmarks_cb (BeditMessageBus *bus,
		           BeditMessage    *message,
		           WindowData      *data)
{
...
static void
message_show_files_cb (BeditMessageBus *bus,
		       BeditMessage    *message,
		       WindowData      *data)
{
...
static void
message_get_view_cb (BeditMessageBus *bus,
		     BeditMessage    *message,
		     WindowData      *data)
{
...
static void
register_methods (BeditWindow            *window,
		  BeditFileBrowserWidget *widget)
{
	...
	bedit_message_bus_register (bus,
...
static void
store_row_inserted (BeditFileBrowserStore *store,
		    GtkTreePath		  *path,
		    GtkTreeIter           *iter,
...
static void
store_before_row_deleted (BeditFileBrowserStore *store,
                          GtkTreePath           *path,
                          MessageCacheData      *data)
{
	GtkTreeIter iter;
...
static void
store_virtual_root_changed (BeditFileBrowserStore *store,
			    GParamSpec            *spec,
			    MessageCacheData      *data)
{
	WindowData *wdata = get_window_data (data->window);
...
static void
store_begin_loading (BeditFileBrowserStore *store,
		     GtkTreeIter           *iter,
		     MessageCacheData      *data)
{
	GtkTreePath *path;
...
static void
store_end_loading (BeditFileBrowserStore *store,
		   GtkTreeIter           *iter,
		   MessageCacheData      *data)
{
	GtkTreePath *path;
...
static void
register_signals (BeditWindow            *window,
		  BeditFileBrowserWidget *widget)
{
	BeditMessageBus *bus = bedit_window_get_message_bus (window);
	BeditFileBrowserStore *store;
...
static void
message_unregistered (BeditMessageBus *bus,
		      ...
		      BeditWindow     *window)
{
	...
	if (data)
	{
		bedit_file_browser_widget_remove_filter (wdata->widget,
		                                         data->id);
	}
...
void
bedit_file_browser_messages_register (BeditWindow            *window,
                                      BeditFileBrowserWidget *widget)
{
	...
	g_signal_connect (bedit_window_get_message_bus (window),
	                  "unregistered",
...
static void
cleanup_signals (BeditWindow *window)
{
	WindowData *data = get_window_data (window);
	BeditFileBrowserStore *store;
...
void
bedit_file_browser_messages_unregister (BeditWindow *window)
{
	BeditMessageBus *bus = bedit_window_get_message_bus (window);
	WindowData *data = get_window_data (window);
