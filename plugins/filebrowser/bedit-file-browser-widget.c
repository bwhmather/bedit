...
static void on_treeview_error                  (BeditFileBrowserView   *tree_view,
						guint                   code,
						gchar                  *message,
						BeditFileBrowserWidget *obj);
static void on_file_store_error                (BeditFileBrowserStore  *store,
						guint                   code,
						gchar                  *message,
						BeditFileBrowserWidget *obj);
static gboolean on_file_store_no_trash 	       (BeditFileBrowserStore  *store,
						GList                  *files,
						BeditFileBrowserWidget *obj);
...
static gboolean
	  on_location_entry_key_press_event    (GtkWidget              *entry,
						GdkEventKey            *event,
						BeditFileBrowserWidget *obj);
static gboolean on_treeview_popup_menu         (BeditFileBrowserView   *treeview,
						BeditFileBrowserWidget *obj);
static gboolean on_treeview_button_press_event (BeditFileBrowserView   *treeview,
						GdkEventButton         *event,
						BeditFileBrowserWidget *obj);
static gboolean on_treeview_key_press_event    (BeditFileBrowserView   *treeview,
						GdkEventKey            *event,
						BeditFileBrowserWidget *obj);
static void on_selection_changed               (GtkTreeSelection       *selection,
						BeditFileBrowserWidget *obj);

static void on_virtual_root_changed            (BeditFileBrowserStore  *model,
						GParamSpec             *param,
						BeditFileBrowserWidget *obj);
...
static void on_bookmarks_row_deleted           (GtkTreeModel           *model,
                                                GtkTreePath            *path,
                                                BeditFileBrowserWidget *obj);
static void on_filter_mode_changed	       (BeditFileBrowserStore  *model,
                                                GParamSpec             *param,
                                                BeditFileBrowserWidget *obj);
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED (BeditFileBrowserWidget,
				bedit_file_browser_widget,
				GTK_TYPE_GRID,
				0,
...
static FilterFunc *
filter_func_new (BeditFileBrowserWidget           *obj,
		 BeditFileBrowserWidgetFilterFunc  func,
		 gpointer                          user_data,
...
static gboolean
locations_find_by_id (BeditFileBrowserWidget *obj,
		      guint                   id,
		      GtkTreeIter            *iter)
{
...
static void
cancel_async_operation (BeditFileBrowserWidget *widget)
{
...
static void
bedit_file_browser_widget_finalize (GObject *object)
{
	BeditFileBrowserWidgetPrivate *priv = GEDIT_FILE_BROWSER_WIDGET (object)->priv;
...
static void
clear_signals (BeditFileBrowserWidget *obj)
{
...
static void
bedit_file_browser_widget_dispose (GObject *object)
{
...
static void
bedit_file_browser_widget_get_property (GObject   *object,
			               guint       prop_id,
			               GValue     *value,
			               GParamSpec *pspec)
{
...
static void
bedit_file_browser_widget_set_property (GObject     *object,
			               guint         prop_id,
			               const GValue *value,
			               GParamSpec   *pspec)
{
	...
	{
		case PROP_FILTER_PATTERN:
			bedit_file_browser_widget_set_filter_pattern (obj,
...
static void
bedit_file_browser_widget_class_init (BeditFileBrowserWidgetClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/com/bwhmather/bedit/plugins/file-browser/ui/bedit-file-browser-widget.ui");
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, previous_button);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, next_button);

	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_button);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_popover);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_treeview);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_treeview_selection);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, treeview_icon_column);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, treeview_icon_renderer);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_cellview);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_button_arrow);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, locations_model);

	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, location_entry);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, treeview);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, filter_entry_revealer);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, filter_entry);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, location_previous_menu);
	gtk_widget_class_bind_template_child_private (widget_class, BeditFileBrowserWidget, location_next_menu);
}

static void
bedit_file_browser_widget_class_finalize (BeditFileBrowserWidgetClass *klass)
{
}

static void
add_signal (BeditFileBrowserWidget *obj,
	    gpointer                object,
	    gulong                  id)
{
...
static gboolean
get_from_bookmark_file (BeditFileBrowserWidget  *obj,
			GFile                   *file,
			gchar                  **name,
			gchar                  **icon_name,
			GdkPixbuf              **icon)
...
static void
insert_path_item (BeditFileBrowserWidget *obj,
                  GFile                  *file,
		  GtkTreeIter            *after,
		  GtkTreeIter            *iter)
{
...
static void
insert_separator_item (BeditFileBrowserWidget *obj)
{
	GtkTreeIter iter;
...
static void
insert_location_path (BeditFileBrowserWidget *obj)
{
	BeditFileBrowserWidgetPrivate *priv = obj->priv;
	Location *loc;
	GFile *current = NULL;
...
static void
remove_path_items (BeditFileBrowserWidget *obj)
{
	GtkTreeIter iter;
...
static void
check_current_item (BeditFileBrowserWidget *obj,
		    gboolean                show_path)
{
	GtkTreeIter separator;
	gboolean has_sep;
...
static void
fill_locations_model (BeditFileBrowserWidget *obj)
{
	BeditFileBrowserWidgetPrivate *priv = obj->priv;
	GtkTreeIter iter;
...
static gboolean
filter_real (BeditFileBrowserStore  *model,
	     GtkTreeIter            *iter,
	     BeditFileBrowserWidget *obj)
{
	GSList *item;
...
static void
add_bookmark_hash (BeditFileBrowserWidget *obj,
		   GtkTreeIter            *iter)
{
	...
	if (!(location = bedit_file_bookmarks_store_get_location (obj->priv->bookmarks_store, iter)))
		return;
...
static void
init_bookmarks_hash (BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = GTK_TREE_MODEL (obj->priv->bookmarks_store);
	GtkTreeIter iter;
...
static void
on_begin_loading (BeditFileBrowserStore  *model,
		  GtkTreeIter            *iter,
		  BeditFileBrowserWidget *obj)
{
...
static void
on_end_loading (BeditFileBrowserStore  *model,
		GtkTreeIter            *iter,
		BeditFileBrowserWidget *obj)
{
	if (!GDK_IS_WINDOW (gtk_widget_get_window (GTK_WIDGET (obj->priv->treeview))))
                                     ...
                                     BeditFileBrowserWidget *obj)
{
	...
	else if (id == PATH_ID)
	{
		...
		bedit_file_browser_store_set_virtual_root_from_location (priv->file_store, file);
...
static void
bedit_file_browser_widget_init (BeditFileBrowserWidget *obj)
{
	GtkBuilder *builder;
	GdkDisplay *display;
	GAction *action;
	...
	if (!gtk_builder_add_from_resource (builder,
	                                    "/com/bwhmather/bedit/plugins/file-browser/ui/bedit-file-browser-menus.ui",
	                                    &error))
	{
	...
	bedit_file_browser_store_set_filter_mode (obj->priv->file_store,
						  GEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN |
						  GEDIT_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY);
	bedit_file_browser_store_set_filter_func (obj->priv->file_store,
...
static void
update_sensitivity (BeditFileBrowserWidget *obj)
{
	...
	{
		mode = bedit_file_browser_store_get_filter_mode (GEDIT_FILE_BROWSER_STORE (model));
...
static gboolean
bedit_file_browser_widget_get_first_selected (BeditFileBrowserWidget *obj,
					      GtkTreeIter            *iter)
{
...
static gboolean
popup_menu (BeditFileBrowserWidget *obj,
...
static gboolean
filter_glob (BeditFileBrowserWidget *obj,
...
static void
rename_selected_file (BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));
	GtkTreeIter iter;
...
static GList *
get_deletable_files (BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));
	GtkTreeSelection *selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->treeview));
	GList *rows = gtk_tree_selection_get_selected_rows (selection, &model);
...
static gboolean
delete_selected_files (BeditFileBrowserWidget *obj,
		       gboolean                trash)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));
	gboolean confirm;
	...
	result = bedit_file_browser_store_delete_all (GEDIT_FILE_BROWSER_STORE (model),
						      rows,
						      trash);
...
static void
show_location_entry (BeditFileBrowserWidget *obj,
...
static gboolean
on_file_store_no_trash (BeditFileBrowserStore  *store,
			GList                  *files,
			BeditFileBrowserWidget *obj)
{
...
static GtkWidget *
create_goto_menu_item (BeditFileBrowserWidget *obj,
		       GList                  *item)
{
...
static void
jump_to_location (BeditFileBrowserWidget *obj,
		  GList                  *item,
		  gboolean                previous)
{
	Location *loc;
	...
	bedit_file_browser_widget_set_root_and_virtual_root (obj,
							     loc->root,
							     loc->virtual_root);
...
static void
clear_next_locations (BeditFileBrowserWidget *obj)
{
	GAction *action;
	GList *children;
	GList *item;
...
static void
update_filter_mode (BeditFileBrowserWidget          *obj,
                    ...
                    BeditFileBrowserStoreFilterMode  mode)
{
	...
	if (GEDIT_IS_FILE_BROWSER_STORE (model))
	{
		gint now = bedit_file_browser_store_get_filter_mode (GEDIT_FILE_BROWSER_STORE (model));
		...
		bedit_file_browser_store_set_filter_mode (GEDIT_FILE_BROWSER_STORE (model), now);
...
static void
set_filter_pattern_real (BeditFileBrowserWidget *obj,
                        gchar const             *pattern,
                        gboolean                 update_entry)
{
	...
	if (pattern == NULL)
	{
		...
		{
			bedit_file_browser_widget_remove_filter (obj, obj->priv->glob_filter_id);
	...
	else
	{
		...
		if (obj->priv->glob_filter_id == 0)
			obj->priv->glob_filter_id = bedit_file_browser_widget_add_filter (obj,
											  filter_glob,
											  NULL,
											  NULL);
	}
...
GtkWidget *
bedit_file_browser_widget_new (void)
{
	BeditFileBrowserWidget *obj = g_object_new (GEDIT_TYPE_FILE_BROWSER_WIDGET, NULL);
...
void
bedit_file_browser_widget_show_bookmarks (BeditFileBrowserWidget *obj)
{
	GtkTreePath *path;
	GtkTreeIter iter;
	...
	bedit_file_browser_view_set_model (obj->priv->treeview,
					   GTK_TREE_MODEL (obj->priv->bookmarks_store));
}

static void
show_files_real (BeditFileBrowserWidget *obj,
...
void
bedit_file_browser_widget_show_files (BeditFileBrowserWidget *obj)
{
	show_files_real (obj, TRUE);
}

void
bedit_file_browser_widget_set_root_and_virtual_root (BeditFileBrowserWidget *obj,
						     GFile                  *root,
						     GFile                  *virtual_root)
{
	BeditFileBrowserStoreResult result;
	...
	if (!virtual_root)
		result = bedit_file_browser_store_set_root_and_virtual_root (obj->priv->file_store,
									     root,
									     root);
	else
		result = bedit_file_browser_store_set_root_and_virtual_root (obj->priv->file_store,
...
void
bedit_file_browser_widget_set_root (BeditFileBrowserWidget *obj,
				    GFile                  *root,
...
gulong
bedit_file_browser_widget_add_filter (BeditFileBrowserWidget           *obj,
				      BeditFileBrowserWidgetFilterFunc  func,
				      gpointer                          user_data,
				      GDestroyNotify                    notify)
{
...
void
bedit_file_browser_widget_remove_filter (BeditFileBrowserWidget *obj,
					 gulong                  id)
{
	GSList *item;
...
void
bedit_file_browser_widget_set_filter_pattern (BeditFileBrowserWidget *obj,
                                              gchar const            *pattern)
{
	gboolean show;
	GAction *action;
...
gboolean
bedit_file_browser_widget_get_selected_directory (BeditFileBrowserWidget *obj,
						  GtkTreeIter            *iter)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));
	GtkTreeIter parent;
...
void
bedit_file_browser_widget_set_active_root_enabled (BeditFileBrowserWidget *widget,
                                                   gboolean                enabled)
{
	GAction *action;
...
static guint
bedit_file_browser_widget_get_num_selected_files_or_directories (BeditFileBrowserWidget *obj,
								 guint                  *files,
								 guint                  *dirs)
{
	GtkTreeSelection *selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->treeview));
...
static AsyncData *
async_data_new (BeditFileBrowserWidget *widget)
{
...
static void
set_busy (BeditFileBrowserWidget *obj,
	  gboolean                busy)
{
...
static void
activate_mount (BeditFileBrowserWidget *widget,
		GVolume		       *volume,
		GMount		       *mount)
{
	GFile *root;
...
static void
try_activate_drive (BeditFileBrowserWidget *widget,
		    GDrive 		   *drive)
{
...
static void
activate_drive (BeditFileBrowserWidget *obj,
		GtkTreeIter	       *iter)
{
	GDrive *drive;
	AsyncData *async;
...
static void
try_mount_volume (BeditFileBrowserWidget *widget,
		  GVolume 		 *volume)
{
...
static void
activate_volume (BeditFileBrowserWidget *obj,
		 GtkTreeIter	        *iter)
{
	GVolume *volume;
...
void
bedit_file_browser_widget_refresh (BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));
...
void
bedit_file_browser_widget_history_back (BeditFileBrowserWidget *obj)
{
	if (obj->priv->locations)
	{
...
void
bedit_file_browser_widget_history_forward (BeditFileBrowserWidget *obj)
{
	if (obj->priv->locations)
...
static void
bookmark_open (BeditFileBrowserWidget *obj,
	       GtkTreeModel           *model,
	       GtkTreeIter            *iter)
{
	GFile *location;
	...
	if ((location = bedit_file_bookmarks_store_get_location (GEDIT_FILE_BOOKMARKS_STORE (model), iter)))
	{
...
static void
file_open (BeditFileBrowserWidget *obj,
	   GtkTreeModel           *model,
	   GtkTreeIter            *iter)
{
	GFile *location;
...
static gboolean
directory_open (BeditFileBrowserWidget *obj,
		GtkTreeModel           *model,
		GtkTreeIter            *iter)
{
...
static void
on_bookmark_activated (BeditFileBrowserView   *tree_view,
		       GtkTreeIter            *iter,
		       BeditFileBrowserWidget *obj)
{
...
static void
on_file_activated (BeditFileBrowserView   *tree_view,
		   GtkTreeIter            *iter,
		   BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (tree_view));
...
static gboolean
virtual_root_is_root (BeditFileBrowserWidget *obj,
...
static void
on_virtual_root_changed (BeditFileBrowserStore  *model,
			 GParamSpec             *param,
			 BeditFileBrowserWidget *obj)
{
	GtkTreeIter iter;
...
static void
on_file_store_error (BeditFileBrowserStore  *store,
		     guint                   code,
		     gchar                  *message,
		     BeditFileBrowserWidget *obj)
{
...
static void
on_treeview_error (BeditFileBrowserView   *tree_view,
		   guint                   code,
		   gchar                  *message,
		   BeditFileBrowserWidget *obj)
{
...
static gboolean
on_treeview_popup_menu (BeditFileBrowserView   *treeview,
			BeditFileBrowserWidget *obj)
{
...
static gboolean
on_treeview_button_press_event (BeditFileBrowserView   *treeview,
				GdkEventButton         *event,
				BeditFileBrowserWidget *obj)
{
...
static gboolean
do_change_directory (BeditFileBrowserWidget *obj,
                     GdkEventKey            *event)
{
	GAction *action = NULL;
...
static gboolean
on_treeview_key_press_event (BeditFileBrowserView   *treeview,
			     GdkEventKey            *event,
			     BeditFileBrowserWidget *obj)
{
	GtkTreeModel *model;
		      ...
		      BeditFileBrowserWidget *obj)
{
	...
	if (GEDIT_IS_FILE_BROWSER_STORE (model))
		selected = bedit_file_browser_widget_get_num_selected_files_or_directories (obj, &files, &dirs);
...
static gboolean
on_entry_filter_activate (BeditFileBrowserWidget *obj)
{
...
static void
on_filter_mode_changed (BeditFileBrowserStore  *model,
                        GParamSpec             *param,
                        BeditFileBrowserWidget *obj)
{
...
next_location_activated (GSimpleAction *action,
                         ...
                         gpointer       user_data)
{
	bedit_file_browser_widget_history_forward (GEDIT_FILE_BROWSER_WIDGET (user_data));
}
...
previous_location_activated (GSimpleAction *action,
                             ...
                             gpointer       user_data)
{
	bedit_file_browser_widget_history_back (GEDIT_FILE_BROWSER_WIDGET (user_data));
}
              ...
              gpointer       user_data)
{
	...
	bedit_file_browser_store_set_virtual_root_up (GEDIT_FILE_BROWSER_STORE (model));
}
                      ...
                      gpointer       user_data)
{
	...
	if (bedit_file_browser_store_new_directory (GEDIT_FILE_BROWSER_STORE (model), &parent, &iter))
                    ...
                    gpointer       user_data)
{
	...
	if (bedit_file_browser_store_new_file (GEDIT_FILE_BROWSER_STORE (model), &parent, &iter))
		bedit_file_browser_view_start_rename (widget->priv->treeview, &iter);
}
                       ...
                       gpointer       user_data)
{
	...
	if (!directory_opened && bedit_file_browser_widget_get_selected_directory (widget, &iter))
...
void
_bedit_file_browser_widget_register_type (GTypeModule *type_module)
{
