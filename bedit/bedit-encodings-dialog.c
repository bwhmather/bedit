...
typedef enum _State { STATE_UNMODIFIED, STATE_MODIFIED, STATE_RESET } State;
...
enum { COLUMN_NAME, COLUMN_CHARSET, COLUMN_ENCODING, N_COLUMNS };

G_DEFINE_TYPE(BeditEncodingsDialog, bedit_encodings_dialog, GTK_TYPE_DIALOG)

static void set_modified(BeditEncodingsDialog *dialog) {
...
static void init_liststores(BeditEncodingsDialog *dialog, gboolean reset) {
...
static GSList *get_chosen_encodings_list(BeditEncodingsDialog *dialog) {
    ...
    while (iter_set) {
        ...
        gtk_tree_model_get(model, &iter, COLUMN_ENCODING, &encoding, -1);
...
static gchar **encoding_list_to_strv(const GSList *enc_list) {
    GSList *l;
    GPtrArray *array;
...
static void update_remove_button_sensitivity(BeditEncodingsDialog *dialog) {
    ...
    for (l = selected_rows; l != NULL; l = l->next) {
         ...
         */
        if (encoding != utf8_encoding && encoding != current_encoding) {
            sensitive = TRUE;
            break;
        }
    }
...
static void update_up_down_buttons_sensitivity(BeditEncodingsDialog *dialog) {
    GtkTreeSelection *selection;
    gint count;
    GList *selected_rows;
    GtkTreeModel *model;
...
static void update_chosen_buttons_sensitivity(BeditEncodingsDialog *dialog) {
...
static void transfer_encodings(
    ...
    for (l = refs; l != NULL; l = l->next) {
        ...
        /* Transfer encoding */
        gtk_tree_model_get(model_orig, &iter, COLUMN_ENCODING, &encoding, -1);
...
static void up_button_clicked_cb(
    ...
    gtk_list_store_move_before(dialog->liststore_chosen, &iter, &prev_iter);
...
static void down_button_clicked_cb(
    ...
    gtk_list_store_move_after(dialog->liststore_chosen, &iter, &next_iter);
...
static void init_toolbar_available(BeditEncodingsDialog *dialog) {
...
static void init_toolbar_chosen(BeditEncodingsDialog *dialog) {
    GtkWidget *scrolled_window;
    GtkToolbar *toolbar;
    GtkStyleContext *context;
    GtkWidget *left_box;
