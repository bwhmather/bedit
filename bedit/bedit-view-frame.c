...
typedef enum { GOTO_LINE, SEARCH } SearchMode;
...
static void hide_search_widget(BeditViewFrame *frame, gboolean cancel) {
    GtkTextBuffer *buffer;
    ...
    if (frame->view_scroll_event_id != 0) {
        g_signal_handler_disconnect(frame->view, frame->view_scroll_event_id);
...
static gboolean search_entry_flush_timeout(BeditViewFrame *frame) {
    frame->flush_timeout_id = 0;
...
static void renew_flush_timeout(BeditViewFrame *frame) {
    ...
    frame->flush_timeout_id = g_timeout_add_seconds(
        FLUSH_TIMEOUT_DURATION, (GSourceFunc)search_entry_flush_timeout, frame);
}

static GtkSourceSearchContext *get_search_context(BeditViewFrame *frame) {
    BeditDocument *doc;
...
static void set_search_state(BeditViewFrame *frame, SearchState state) {
...
static void finish_search(BeditViewFrame *frame, gboolean found) {
    const gchar *entry_text =
        gtk_entry_get_text(GTK_ENTRY(frame->search_entry));
...
static void start_search_finished(
    ...
    if (found) {
        ...
        gtk_text_buffer_get_iter_at_mark(
            GTK_TEXT_BUFFER(buffer), &start_at, frame->start_mark);
...
static void start_search(BeditViewFrame *frame) {
    GtkSourceSearchContext *search_context;
    GtkTextIter start_at;
    ...
    gtk_source_search_context_forward_async(
        ...
        (GAsyncReadyCallback)start_search_finished, frame);
}

static void forward_search_finished(
    GtkSourceSearchContext *search_context, GAsyncResult *result,
    BeditViewFrame *frame) {
    GtkTextIter match_start;
    GtkTextIter match_end;
    ...
    if (found) {
        GtkTextBuffer *buffer =
            gtk_text_view_get_buffer(GTK_TEXT_VIEW(frame->view));
...
static void forward_search(BeditViewFrame *frame) {
    GtkTextIter start_at;
    GtkTextBuffer *buffer;
    GtkSourceSearchContext *search_context;
    ...
    gtk_source_search_context_forward_async(
        search_context, &start_at, NULL,
        (GAsyncReadyCallback)forward_search_finished, frame);
}

static void backward_search_finished(
    GtkSourceSearchContext *search_context, GAsyncResult *result,
    BeditViewFrame *frame) {
    GtkTextIter match_start;
    GtkTextIter match_end;
...
static void backward_search(BeditViewFrame *frame) {
    GtkTextIter start_at;
    GtkTextBuffer *buffer;
    GtkSourceSearchContext *search_context;
    ...
    gtk_source_search_context_backward_async(
        search_context, &start_at, NULL,
        (GAsyncReadyCallback)backward_search_finished, frame);
}

static gboolean search_widget_scroll_event(
    GtkWidget *widget, GdkEventScroll *event, BeditViewFrame *frame) {
...
static GtkSourceSearchSettings *copy_search_settings(
...
static gboolean search_widget_key_press_event(
...
static gboolean remove_entry_tag_timeout_cb(BeditViewFrame *frame) {
    frame->remove_entry_tag_timeout_id = 0;

    gd_tagged_entry_remove_tag(frame->search_entry, frame->entry_tag);

    return G_SOURCE_REMOVE;
}

static void update_entry_tag(BeditViewFrame *frame) {
    GtkSourceSearchContext *search_context;
    GtkTextBuffer *buffer;
    ...
    pos = gtk_source_search_context_get_occurrence_position(
        search_context, &select_start, &select_end);
    ...
    if (count == -1 || pos == -1) {
        ...
        if (frame->remove_entry_tag_timeout_id == 0) {
            frame->remove_entry_tag_timeout_id = g_timeout_add(
                500, (GSourceFunc)remove_entry_tag_timeout_cb, frame);
        }
    ...
    gd_tagged_entry_add_tag(frame->search_entry, frame->entry_tag);

    g_free(label);
}

static gboolean update_entry_tag_idle_cb(BeditViewFrame *frame) {
    frame->idle_update_entry_tag_id = 0;
...
static void install_update_entry_tag_idle(BeditViewFrame *frame) {
    if (frame->idle_update_entry_tag_id == 0) {
        frame->idle_update_entry_tag_id =
            g_idle_add((GSourceFunc)update_entry_tag_idle_cb, frame);
    }
}

static void update_search_text(BeditViewFrame *frame) {
    const gchar *entry_text =
        gtk_entry_get_text(GTK_ENTRY(frame->search_entry));
    ...
    if (gtk_source_search_settings_get_regex_enabled(frame->search_settings)) {
        gtk_source_search_settings_set_search_text(
            frame->search_settings, entry_text);
    } else {
        gchar *unescaped_entry_text =
            gtk_source_utils_unescape_search_text(entry_text);

        gtk_source_search_settings_set_search_text(
            frame->search_settings, unescaped_entry_text);

        g_free(unescaped_entry_text);
    }
}

static void regex_toggled_cb(
    GtkCheckMenuItem *menu_item, BeditViewFrame *frame) {
    gtk_source_search_settings_set_regex_enabled(
        frame->search_settings, gtk_check_menu_item_get_active(menu_item));

    start_search(frame);
}

static void at_word_boundaries_toggled_cb(
    GtkCheckMenuItem *menu_item, BeditViewFrame *frame) {
...
static void case_sensitive_toggled_cb(
    GtkCheckMenuItem *menu_item, BeditViewFrame *frame) {
...
static void add_popup_menu_items(BeditViewFrame *frame, GtkWidget *menu) {
    GtkWidget *menu_item;
    gboolean val;
    ...
    g_object_bind_property(
        frame->search_settings, "wrap-around", menu_item, "active",
    ...
    g_signal_connect(menu_item, "toggled", G_CALLBACK(regex_toggled_cb), frame);

    /* create "Match Entire Word Only" menu item. */
    menu_item =
        gtk_check_menu_item_new_with_mnemonic(_("Match _Entire Word Only"));

    gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), menu_item);
    gtk_widget_show(menu_item);

    val = gtk_source_search_settings_get_at_word_boundaries(
        frame->search_settings);
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menu_item), val);

    g_signal_connect(
        menu_item, "toggled", G_CALLBACK(at_word_boundaries_toggled_cb), frame);
    ...
    g_signal_connect(
        menu_item, "toggled", G_CALLBACK(case_sensitive_toggled_cb), frame);
}

static void popup_menu_hide_cb(BeditViewFrame *frame) {
    renew_flush_timeout(frame);

    g_signal_handler_unblock(
        frame->search_entry, frame->search_entry_focus_out_id);
}

static void setup_popup_menu(BeditViewFrame *frame, GtkWidget *menu) {
    if (frame->flush_timeout_id != 0) {
        g_source_remove(frame->flush_timeout_id);
        frame->flush_timeout_id = 0;
    }
    ...
    g_signal_connect_swapped(
        menu, "hide", G_CALLBACK(popup_menu_hide_cb), frame);
}
...
static void search_entry_escaped(GtkSearchEntry *entry, BeditViewFrame *frame) {
    ...
    if (frame->search_mode == SEARCH && search_context != NULL) {
        ...
        g_clear_object(&frame->search_settings);
        frame->search_settings =
            copy_search_settings(frame->old_search_settings);
...
static void search_entry_previous_match(
    GtkSearchEntry *entry, BeditViewFrame *frame) {
    backward_search(frame);
}

static void search_entry_next_match(
    GtkSearchEntry *entry, BeditViewFrame *frame) {
    forward_search(frame);
}

static void search_entry_populate_popup(
    GtkEntry *entry, GtkMenu *menu, BeditViewFrame *frame) {
    GtkWidget *menu_item;
...
static void search_entry_icon_release(
    GtkEntry *entry, GtkEntryIconPosition icon_pos, GdkEventButton *event,
    BeditViewFrame *frame) {
    GtkWidget *menu;

    if (frame->search_mode == GOTO_LINE || icon_pos != GTK_ENTRY_ICON_PRIMARY) {
        return;
    }
    ...
    gtk_menu_popup_at_widget(
        GTK_MENU(menu), GTK_WIDGET(entry), GDK_GRAVITY_SOUTH_WEST,
        GDK_GRAVITY_NORTH_WEST, NULL);
}

static void search_entry_activate(GtkEntry *entry, BeditViewFrame *frame) {
    hide_search_widget(frame, FALSE);
    gtk_widget_grab_focus(GTK_WIDGET(frame->view));
}
...
static void customize_for_search_mode(BeditViewFrame *frame) {
    GIcon *icon;
    gint width_request;
    ...
    gtk_entry_set_icon_from_gicon(
        GTK_ENTRY(frame->search_entry), GTK_ENTRY_ICON_PRIMARY, icon);

    gtk_widget_set_size_request(
        GTK_WIDGET(frame->search_entry), width_request, -1);
...
static void search_entry_changed_cb(GtkEntry *entry, BeditViewFrame *frame) {
    renew_flush_timeout(frame);
...
static gboolean search_entry_focus_out_event(
    GtkWidget *widget, GdkEventFocus *event, BeditViewFrame *frame) {
    hide_search_widget(frame, FALSE);
    return GDK_EVENT_PROPAGATE;
}

static void mark_set_cb(
...
static gboolean get_selected_text(
    GtkTextBuffer *doc, gchar **selected_text, gint *len) {
    GtkTextIter start;
    GtkTextIter end;
...
static void init_search_entry(BeditViewFrame *frame) {
    if (frame->search_mode == GOTO_LINE) {
        gint line;
        gchar *line_str;
        GtkTextIter iter;
        ...
        gtk_editable_select_region(GTK_EDITABLE(frame->search_entry), 0, -1);
        ...
        if (frame->search_settings == NULL) {
            ...
            gtk_source_search_settings_set_wrap_around(
                frame->search_settings, TRUE);
        }

        g_clear_object(&frame->old_search_settings);
        frame->old_search_settings =
            copy_search_settings(frame->search_settings);
        ...
        if (search_context == NULL) {
            ...
            g_signal_connect_swapped(
                ...
                G_CALLBACK(install_update_entry_tag_idle), frame);

            g_object_unref(search_context);
        }

        selection_exists =
            get_selected_text(buffer, &search_text, &selection_len);

        if (selection_exists && (search_text != NULL) &&
            (selection_len <= 160)) {
            gchar *search_text_escaped;

            if (gtk_source_search_settings_get_regex_enabled(
                    frame->search_settings)) {
                search_text_escaped = g_regex_escape_string(search_text, -1);
            } else {
                search_text_escaped =
                    gtk_source_utils_escape_search_text(search_text);
            }
            ...
            if (g_strcmp0(search_text_escaped, frame->search_text) == 0) {
                 ...
                 */
                g_signal_handler_block(
                    frame->search_entry, frame->search_entry_changed_id);

                gtk_entry_set_text(
                    GTK_ENTRY(frame->search_entry), search_text_escaped);

                gtk_editable_select_region(
                    GTK_EDITABLE(frame->search_entry), 0, -1);
                ...
                gtk_editable_set_position(
                    GTK_EDITABLE(frame->search_entry), -1);
            }
            ...
            gtk_entry_set_text(
                GTK_ENTRY(frame->search_entry), frame->search_text);
...
static void start_interactive_search_real(
    BeditViewFrame *frame, SearchMode request_search_mode) {
    GtkTextBuffer *buffer;
    GtkTextIter iter;
    ...
    frame->view_scroll_event_id = g_signal_connect(
        frame->view, "scroll-event", G_CALLBACK(search_widget_scroll_event),
        frame);
...
static void bedit_view_frame_class_init(BeditViewFrameClass *klass) {
    ...
    gtk_widget_class_bind_template_child(
        widget_class, BeditViewFrame, search_entry);
    gtk_widget_class_bind_template_child(
        widget_class, BeditViewFrame, go_up_button);
    gtk_widget_class_bind_template_child(
        widget_class, BeditViewFrame, go_down_button);
}
...
static void bedit_view_frame_init(BeditViewFrame *frame) {
    ...
    gd_tagged_entry_tag_set_style(
        frame->entry_tag, "bedit-search-entry-occurrences-tag");
    ...
    g_signal_connect(doc, "mark-set", G_CALLBACK(mark_set_cb), frame);
    ...
    g_signal_connect(
        frame->revealer, "scroll-event", G_CALLBACK(search_widget_scroll_event),
        frame);

    g_signal_connect(
        frame->search_entry, "populate-popup",
        G_CALLBACK(search_entry_populate_popup), frame);

    g_signal_connect(
        frame->search_entry, "icon-release",
        G_CALLBACK(search_entry_icon_release), frame);

    g_signal_connect(
        frame->search_entry, "activate", G_CALLBACK(search_entry_activate),
        frame);

    g_signal_connect(
        frame->search_entry, "insert-text",
        G_CALLBACK(search_entry_insert_text), frame);

    g_signal_connect(
        frame->search_entry, "stop-search", G_CALLBACK(search_entry_escaped),
        frame);

    g_signal_connect(
        frame->search_entry, "next-match", G_CALLBACK(search_entry_next_match),
        frame);

    g_signal_connect(
        frame->search_entry, "previous-match",
        G_CALLBACK(search_entry_previous_match), frame);

    frame->search_entry_changed_id = g_signal_connect(
        frame->search_entry, "changed", G_CALLBACK(search_entry_changed_cb),
        frame);

    frame->search_entry_focus_out_id = g_signal_connect(
        frame->search_entry, "focus-out-event",
        G_CALLBACK(search_entry_focus_out_event), frame);

    g_signal_connect_swapped(
        frame->go_up_button, "clicked", G_CALLBACK(backward_search), frame);

    g_signal_connect_swapped(
        frame->go_down_button, "clicked", G_CALLBACK(forward_search), frame);
}
