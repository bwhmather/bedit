...
static void tab_state_changed_while_saving (BeditTab    *tab,
					    GParamSpec  *pspec,
					    BeditWindow *window);

void
_bedit_cmd_file_new (GSimpleAction *action,
                     GVariant      *parameter,
                     gpointer       user_data)
{
...
/* File loading */
static GSList *
load_file_list (BeditWindow             *window,
		const GSList            *files,
		...
		gboolean                 create)
{
		...
		else
		{
			if (l == files)
			{
				...
				if (line_pos > 0)
				{
					if (column_pos > 0)
					{
						bedit_document_goto_line_offset (doc,
						                                 line_pos - 1,
						                                 column_pos - 1);
					}
					else
	...
	while (l != NULL)
	{
		...
		tab = bedit_window_create_tab_from_location (window,
							     l->data,
							     encoding,
							     line_pos,
							     column_pos,
 ...
 */
void
bedit_commands_load_location (BeditWindow             *window,
			      GFile                   *location,
			      const GtkSourceEncoding *encoding,
			      gint                     line_pos,
			      gint                     column_pos)
 ...
 */
GSList *
bedit_commands_load_locations (BeditWindow             *window,
 ...
 */
GSList *
_bedit_cmd_load_files_from_prompt (BeditWindow             *window,
				   GSList                  *files,
				   const GtkSourceEncoding *encoding,
				   gint                     line_pos,
				   gint                     column_pos)
...
static void
open_dialog_destroyed (BeditWindow            *window,
...
static void
open_dialog_response_cb (BeditFileChooserDialog *dialog,
                         gint                    response_id,
                         BeditWindow            *window)
{
	GSList *files;
	...
	if (window == NULL)
	{
		window = bedit_app_create_window (GEDIT_APP (g_application_get_default ()),
		                                  NULL);
...
void
_bedit_cmd_file_open (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	...
	open_dialog = bedit_file_chooser_dialog_create (C_("window title", "Open"),
	...
	if (window != NULL)
	{
		...
		if (default_path != NULL)
		{
			bedit_file_chooser_dialog_set_current_folder (open_dialog, default_path);
...
void
_bedit_cmd_file_reopen_closed_tab (GSimpleAction *action,
				   GVariant      *parameter,
				   gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
...
static void
tab_save_as_ready_cb (BeditTab     *tab,
		      GAsyncResult *result,
		      GTask        *task)
{
...
static void
save_dialog_response_cb (BeditFileChooserDialog *dialog,
			 gint                    response_id,
			 GTask                  *task)
{
	BeditTab *tab;
...
confirm_overwrite_callback (BeditFileChooserDialog *dialog,
			    gpointer                data)
{
...
static void
save_as_tab_async (BeditTab            *tab,
		   BeditWindow         *window,
		   ...
		   gpointer             user_data)
{
	...
	save_dialog = bedit_file_chooser_dialog_create (C_("window title", "Save As"),
							GTK_WINDOW (window),
	...
	else
	{
		...
		if (default_path != NULL)
		{
			bedit_file_chooser_dialog_set_current_folder (save_dialog,
								      default_path);

			g_object_unref (default_path);
		}

		bedit_file_chooser_dialog_set_current_name (save_dialog,
							    docname);
	...
	bedit_file_chooser_dialog_set_encoding (GEDIT_FILE_CHOOSER_DIALOG (save_dialog),
						encoding);

	bedit_file_chooser_dialog_set_newline_type (GEDIT_FILE_CHOOSER_DIALOG (save_dialog),
	                                            newline_type);
...
static gboolean
save_as_tab_finish (BeditTab     *tab,
		    GAsyncResult *result)
{
...
static void
save_as_tab_ready_cb (BeditTab     *tab,
		      GAsyncResult *result,
		      GTask        *task)
{
...
static void
tab_save_ready_cb (BeditTab     *tab,
		   GAsyncResult *result,
		   GTask        *task)
{
	gboolean success = _bedit_tab_save_finish (tab, result);
 ...
 */
void
bedit_commands_save_document_async (BeditDocument       *document,
				    BeditWindow         *window,
				    GCancellable        *cancellable,
				    GAsyncReadyCallback  callback,
				    gpointer             user_data)
 ...
 */
gboolean
bedit_commands_save_document_finish (BeditDocument *document,
				     GAsyncResult  *result)
{
...
static void
save_tab_ready_cb (BeditDocument *doc,
		   GAsyncResult  *result,
		   gpointer       user_data)
{
...
static void
save_tab (BeditTab    *tab,
	  BeditWindow *window)
{
	BeditDocument *doc = bedit_tab_get_document (tab);

	bedit_commands_save_document_async (doc,
					    window,
					    NULL,
...
void
_bedit_cmd_file_save (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
...
static void
_bedit_cmd_file_save_as_cb (BeditTab     *tab,
			    GAsyncResult *result,
			    gpointer      user_data)
{
...
void
_bedit_cmd_file_save_as (GSimpleAction *action,
                         GVariant      *parameter,
                         gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
	...
	if (tab != NULL)
	{
		save_as_tab_async (tab,
				   window,
				   NULL,
				   (GAsyncReadyCallback) _bedit_cmd_file_save_as_cb,
				   NULL);
	}
}

static void
quit_if_needed (BeditWindow *window)
{
...
really_close_tab (BeditTab *tab)
{
	...
	g_return_val_if_fail (bedit_tab_get_state (tab) == GEDIT_TAB_STATE_CLOSING,
			      FALSE);
...
static void
save_as_documents_list_cb (BeditTab     *tab,
			   GAsyncResult *result,
			   SaveAsData   *data)
{
 ...
 */
static void
save_documents_list (BeditWindow *window,
		     GList       *docs)
{
	...
	g_return_if_fail ((bedit_window_get_state (window) & GEDIT_WINDOW_STATE_PRINTING) == 0);
	...
	for (l = docs; l != NULL; l = l->next)
	{
		...
		else
		{
			...
			*/
			...
			uri_for_display = bedit_document_get_uri_for_display (doc);
			bedit_debug_message (DEBUG_COMMANDS,
 ...
 */
void
bedit_commands_save_all_documents (BeditWindow *window)
{
	GList *docs;
...
void
_bedit_cmd_file_save_all (GSimpleAction *action,
                          GVariant      *parameter,
                          gpointer       user_data)
{
 ...
 */
void
bedit_commands_save_document (BeditWindow   *window,
...
static GtkWidget *
revert_dialog (BeditWindow   *window,
	       BeditDocument *doc)
{
	GtkWidget *dialog;
	gchar *docname;
...
void
_bedit_cmd_file_revert (GSimpleAction *action,
                        GVariant      *parameter,
                        gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
	 ...
	 * the user further.
	 */
	if (bedit_tab_get_state (tab) == GEDIT_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION ||
...
static void
tab_state_changed_while_saving (BeditTab    *tab,
				GParamSpec  *pspec,
				BeditWindow *window)
{
	BeditTabState state;
...
static void
close_all_tabs (BeditWindow *window)
{
	bedit_debug (DEBUG_COMMANDS);
...
static void
close_document (BeditWindow   *window,
		BeditDocument *doc)
{
	BeditTab *tab;
...
static void
close_confirmation_dialog_response_handler (BeditCloseConfirmationDialog *dlg,
                                            gint                          response_id,
                                            BeditWindow                  *window)
{
	...
	switch (response_id)
	{
		...
		case GTK_RESPONSE_YES:
			selected_documents = bedit_close_confirmation_dialog_get_selected_documents (dlg);
			...
			else
			{
				...
				{
					BeditNotebook *notebook = is_closing_all ? NULL : notebook_to_close;
		...
		case GTK_RESPONSE_NO:
			...
			else
			{
				...
				unsaved_documents = bedit_close_confirmation_dialog_get_unsaved_documents (dlg);
 ...
 */
void
_bedit_cmd_file_close_tab (BeditTab    *tab,
			   BeditWindow *window)
{
	bedit_debug (DEBUG_COMMANDS);
...
void
_bedit_cmd_file_close (GSimpleAction *action,
                       GVariant      *parameter,
                       gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
...
static void
file_close_dialog (BeditWindow *window,
                   GList       *unsaved_docs)
{
	GtkWidget *dlg;
	...
	if (unsaved_docs->next == NULL)
	{
		...
		dlg = bedit_close_confirmation_dialog_new_single (GTK_WINDOW (window), doc);
	}
	else
	{
		dlg = bedit_close_confirmation_dialog_new (GTK_WINDOW (window), unsaved_docs);
	}
...
static GList *
notebook_get_unsaved_documents (BeditNotebook *notebook)
{
	GList *children;
...
/* Close a notebook */
void
_bedit_cmd_file_close_notebook (BeditWindow   *window,
                                BeditNotebook *notebook)
{
	GList *unsaved_docs;
...
/* Close all tabs */
static void
file_close_all (BeditWindow *window,
                gboolean     is_quitting)
{
	GList *unsaved_docs;
	...
	g_return_if_fail (!(bedit_window_get_state (window) &
...
void
_bedit_cmd_file_close_all (GSimpleAction *action,
                           GVariant      *parameter,
                           gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
...
void
_bedit_cmd_file_quit (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	BeditWindow *window = GEDIT_WINDOW (user_data);
