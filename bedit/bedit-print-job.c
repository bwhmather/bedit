...
static void bedit_print_job_class_init(BeditPrintJobClass *klass) {
    ...
    signals[PRINTING] = g_signal_new_class_handler(
        ...
        G_CALLBACK(bedit_print_job_printing), NULL, NULL, NULL, G_TYPE_NONE, 1,
        G_TYPE_UINT);
    ...
    signals[DONE] = g_signal_new_class_handler(
        ...
        G_CALLBACK(bedit_print_job_done), NULL, NULL, NULL, G_TYPE_NONE, 2,
...
static GObject *create_custom_widget_cb(
    ...
    g_object_bind_property(
        job->line_numbers_checkbutton, "active", line_numbers_hbox, "sensitive",
    ...
    g_object_bind_property(
        job->text_wrapping_checkbutton, "active", job->do_not_split_checkbutton,
        "sensitive", G_BINDING_SYNC_CREATE);

    g_object_bind_property(
        job->text_wrapping_checkbutton, "active", job->do_not_split_checkbutton,
        "inconsistent", G_BINDING_INVERT_BOOLEAN | G_BINDING_SYNC_CREATE);

    /* Restore */
    g_signal_connect(
        restore_button, "clicked", G_CALLBACK(restore_button_clicked), job);
...
static void custom_widget_apply_cb(
    ...
    body_font =
        gtk_font_chooser_get_font(GTK_FONT_CHOOSER(job->body_fontbutton));
    header_font =
        gtk_font_chooser_get_font(GTK_FONT_CHOOSER(job->headers_fontbutton));
    numbers_font =
...
static void create_compositor(BeditPrintJob *job) {
    ...
    job->compositor = GTK_SOURCE_PRINT_COMPOSITOR(g_object_new(
        GTK_SOURCE_TYPE_PRINT_COMPOSITOR, "buffer", buf, "tab-width", tab_width,
        "highlight-syntax", syntax_hl, "wrap-mode", wrap_mode,
        "print-line-numbers", print_line_numbers, "print-header", print_header,
        "print-footer", FALSE, "body-font-name", print_font_body,
        "line-numbers-font-name", print_font_numbers, "header-font-name",
...
static gboolean paginate_cb(
    ...
    job->progress =
        gtk_source_print_compositor_get_pagination_progress(job->compositor);
...
GtkPrintOperationResult bedit_print_job_print(
    ...
    g_signal_connect(job->operation, "preview", G_CALLBACK(preview_cb), job);

    g_signal_connect(
        job->operation, "begin-print", G_CALLBACK(begin_print_cb), job);

    g_signal_connect(job->operation, "paginate", G_CALLBACK(paginate_cb), job);

    g_signal_connect(
        job->operation, "draw-page", G_CALLBACK(draw_page_cb), job);

    g_signal_connect_object(
        job->operation, "end-print", G_CALLBACK(end_print_cb), job, 0);

    g_signal_connect_object(
        job->operation, "done", G_CALLBACK(done_cb), job, 0);
