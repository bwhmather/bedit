...
#define BEDIT_TYPE_DOCUMENTS_GROUP_ROW (bedit_documents_group_row_get_type())
#define BEDIT_DOCUMENTS_GROUP_ROW(obj)                                         \
    (G_TYPE_CHECK_INSTANCE_CAST(                                               \
        (obj), BEDIT_TYPE_DOCUMENTS_GROUP_ROW, BeditDocumentsGroupRow))
#define BEDIT_DOCUMENTS_GROUP_ROW_CLASS(klass)                                 \
    (G_TYPE_CHECK_CLASS_CAST(                                                  \
        (klass), BEDIT_TYPE_DOCUMENTS_GROUP_ROW, BeditDocumentsGroupRowClass))
#define BEDIT_IS_DOCUMENTS_GROUP_ROW(obj)                                      \
    (G_TYPE_CHECK_INSTANCE_TYPE((obj), BEDIT_TYPE_DOCUMENTS_GROUP_ROW))
#define BEDIT_IS_DOCUMENTS_GROUP_ROW_CLASS(klass)                              \
    (G_TYPE_CHECK_CLASS_TYPE((klass), BEDIT_TYPE_DOCUMENTS_GROUP_ROW))
#define BEDIT_DOCUMENTS_GROUP_ROW_GET_CLASS(obj)                               \
    (G_TYPE_INSTANCE_GET_CLASS(                                                \
        (obj), BEDIT_TYPE_DOCUMENTS_GROUP_ROW, BeditDocumentsGroupRowClass))
...
G_DEFINE_TYPE(
    BeditDocumentsGroupRow, bedit_documents_group_row, GTK_TYPE_LIST_BOX_ROW)

#define BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW                                      \
    (bedit_documents_document_row_get_type())
#define BEDIT_DOCUMENTS_DOCUMENT_ROW(obj)                                      \
    (G_TYPE_CHECK_INSTANCE_CAST(                                               \
        (obj), BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW, BeditDocumentsDocumentRow))
#define BEDIT_DOCUMENTS_DOCUMENT_ROW_CLASS(klass)                              \
    (G_TYPE_CHECK_CLASS_CAST(                                                  \
        (klass), BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW,                            \
        BeditDocumentsDocumentRowClass))
#define BEDIT_IS_DOCUMENTS_DOCUMENT_ROW(obj)                                   \
    (G_TYPE_CHECK_INSTANCE_TYPE((obj), BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW))
#define BEDIT_IS_DOCUMENTS_DOCUMENT_ROW_CLASS(klass)                           \
    (G_TYPE_CHECK_CLASS_TYPE((klass), BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW))
#define BEDIT_DOCUMENTS_DOCUMENT_ROW_GET_CLASS(obj)                            \
    (G_TYPE_INSTANCE_GET_CLASS(                                                \
        (obj), BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW,                              \
        BeditDocumentsDocumentRowClass))
...
G_DEFINE_TYPE(
    BeditDocumentsDocumentRow, bedit_documents_document_row,
    GTK_TYPE_LIST_BOX_ROW)

static GtkWidget *bedit_documents_document_row_new(
    BeditDocumentsPanel *panel, BeditTab *tab);
static GtkWidget *bedit_documents_group_row_new(
    BeditDocumentsPanel *panel, BeditNotebook *notebook);
...
struct _BeditDocumentsPanel {
    ...
    /* Flag to workaround first GroupRow selection at start ( we don't want to
     * show it ) */
...
static const GtkTargetEntry panel_targets[] = {
    {"BEDIT_DOCUMENTS_DOCUMENT_ROW", GTK_TARGET_SAME_APP, 0},
};
...
static guint get_nb_visible_rows(BeditDocumentsPanel *panel) {
    guint nb = 0;
...
static guint get_row_visible_index(
    BeditDocumentsPanel *panel, GtkWidget *searched_row) {
    GList *children;
    GList *l;
    ...
    for (l = children; l != NULL; l = g_list_next(l)) {
        ...
        if (BEDIT_IS_DOCUMENTS_GROUP_ROW(row)) {
            nb_notebook_row += 1;
        } else {
            nb_tab_row += 1;
        }
...
static void make_row_visible(BeditDocumentsPanel *panel, GtkWidget *row) {
    ...
    if (row_position < adjustment_value) {
        ...
        (row_position + row_size) > (adjustment_value + adjustment_page_size)) {
        offset = (row_position + row_size) -
            (adjustment_value + adjustment_page_size);
...
static gint listbox_search_function(gconstpointer row, gconstpointer item) {
...
static GtkListBoxRow *get_row_from_widget(
    BeditDocumentsPanel *panel, GtkWidget *widget) {
    GList *children;
    GList *item;
    GtkListBoxRow *row;
...
static void row_select(
    BeditDocumentsPanel *panel, GtkListBox *listbox, GtkListBoxRow *row) {
...
static void insert_row(
    BeditDocumentsPanel *panel, GtkListBox *listbox, GtkWidget *row,
    gint position) {
    g_signal_handler_block(listbox, panel->selection_changed_handler_id);
...
static void select_active_tab(BeditDocumentsPanel *panel) {
...
static GtkListBoxRow *get_first_notebook_found(BeditDocumentsPanel *panel) {
    GList *children;
    GList *l;
    ...
    for (l = children; l != NULL; l = g_list_next(l)) {
        if (BEDIT_IS_DOCUMENTS_GROUP_ROW(l->data)) {
            row = l->data;
            break;
        }
    }
...
static void multi_notebook_tab_switched(
...
static void group_row_set_notebook_name(GtkWidget *row) {
    BeditNotebook *notebook;
    BeditMultiNotebook *mnb;
    guint num;
    gchar *name;
    BeditDocumentsGroupRow *group_row = BEDIT_DOCUMENTS_GROUP_ROW(row);

    notebook = BEDIT_NOTEBOOK(group_row->ref);
...
static void group_row_update_names(
    BeditDocumentsPanel *panel, GtkWidget *listbox) {
    GList *children;
    GList *l;
    GtkWidget *row;
...
static void group_row_refresh_visibility(BeditDocumentsPanel *panel) {
...
static gchar *doc_get_name(BeditDocument *doc) {
    gchar *name;
    gchar *docname;
...
static void document_row_sync_tab_name_and_icon(
    BeditTab *tab, GParamSpec *pspec, GtkWidget *row) {
    BeditDocumentsDocumentRow *document_row = BEDIT_DOCUMENTS_DOCUMENT_ROW(row);
    BeditDocument *doc;
    GtkSourceFile *file;
    gchar *name;
    GdkPixbuf *pixbuf;
...
static void refresh_notebook(
    ...
    for (l = tabs; l != NULL; l = g_list_next(l)) {
        ...
        row = bedit_documents_document_row_new(panel, BEDIT_TAB(l->data));
...
static void refresh_notebook_foreach(
    BeditNotebook *notebook, BeditDocumentsPanel *panel) {
    GtkWidget *row;
...
static void refresh_list(BeditDocumentsPanel *panel) {
    GList *children;
    GList *l;
    ...
    for (l = children; l != NULL; l = g_list_next(l)) {
        ...
        if (BEDIT_IS_DOCUMENTS_DOCUMENT_ROW(row)) {
            BeditTab *tab = BEDIT_TAB(row->ref);
            g_signal_handlers_disconnect_matched(
                tab, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                G_CALLBACK(document_row_sync_tab_name_and_icon), NULL);
        }
    ...
    bedit_multi_notebook_foreach_notebook(
        panel->mnb, (GtkCallback)refresh_notebook_foreach, panel);
    select_active_tab(panel);
}

static void multi_notebook_tab_removed(
    ...
    g_signal_handlers_disconnect_by_func(
        BEDIT_DOCUMENTS_DOCUMENT_ROW(row)->ref,
...
static gint get_dest_position_for_tab(
    BeditDocumentsPanel *panel, BeditNotebook *notebook, BeditTab *tab) {
    gint page_num;
    GList *children;
    GList *item;
    gint res = -1;
...
static void multi_notebook_tab_added(
    BeditMultiNotebook *mnb, BeditNotebook *notebook, BeditTab *tab,
    BeditDocumentsPanel *panel) {
    gint position;
    GtkWidget *row;
    ...
    if (position == -1) {
        ...
        if (tab == bedit_multi_notebook_get_active_tab(mnb)) {
            row_select(
                panel, GTK_LIST_BOX(panel->listbox), GTK_LIST_BOX_ROW(row));
        }
    }
}

static void multi_notebook_notebook_removed(
...
static void row_move(
    BeditDocumentsPanel *panel, BeditNotebook *notebook, GtkWidget *tab,
    GtkWidget *row) {
    gint position;
    ...
    gtk_container_remove(GTK_CONTAINER(panel->listbox), GTK_WIDGET(row));
    position = get_dest_position_for_tab(panel, notebook, BEDIT_TAB(tab));
    ...
    g_signal_handler_unblock(
        GTK_LIST_BOX(panel->listbox), panel->selection_changed_handler_id);
}

static void multi_notebook_tabs_reordered(
    BeditMultiNotebook *mnb, BeditNotebook *notebook, GtkWidget *page,
    gint page_num, BeditDocumentsPanel *panel) {
    GtkListBoxRow *row;
...
static void set_window(BeditDocumentsPanel *panel, BeditWindow *window) {
    panel->window = g_object_ref(window);
    panel->mnb = BEDIT_MULTI_NOTEBOOK(_bedit_window_get_multi_notebook(window));

    g_signal_connect(
        panel->mnb, "notebook-removed",
        G_CALLBACK(multi_notebook_notebook_removed), panel);
    g_signal_connect(
        panel->mnb, "tab-added", G_CALLBACK(multi_notebook_tab_added), panel);
    g_signal_connect(
        panel->mnb, "tab-removed", G_CALLBACK(multi_notebook_tab_removed),
        panel);
    g_signal_connect(
        panel->mnb, "page-reordered", G_CALLBACK(multi_notebook_tabs_reordered),
        panel);

    panel->tab_switched_handler_id = g_signal_connect(
        panel->mnb, "switch-tab", G_CALLBACK(multi_notebook_tab_switched),
        panel);
...
static void listbox_selection_changed(
    GtkListBox *listbox, GtkListBoxRow *row, BeditDocumentsPanel *panel) {
    if (row == NULL) {
    ...
    if (BEDIT_IS_DOCUMENTS_DOCUMENT_ROW(row)) {
        bedit_multi_notebook_set_active_tab(
            panel->mnb, BEDIT_TAB(BEDIT_DOCUMENTS_DOCUMENT_ROW(row)->ref));

        panel->current_selection = GTK_WIDGET(row);
    } else if (BEDIT_IS_DOCUMENTS_GROUP_ROW(row) && panel->current_selection) {
        row_select(
            panel, GTK_LIST_BOX(panel->listbox),
...
static void bedit_documents_panel_set_property(
    GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(object);
...
static void bedit_documents_panel_get_property(
...
static void bedit_documents_panel_finalize(GObject *object) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(object);

    g_signal_handlers_disconnect_by_func(
        panel->mnb, G_CALLBACK(multi_notebook_notebook_removed), panel);
    g_signal_handlers_disconnect_by_func(
        panel->mnb, G_CALLBACK(multi_notebook_tab_added), panel);
    g_signal_handlers_disconnect_by_func(
        panel->mnb, G_CALLBACK(multi_notebook_tab_removed), panel);
    g_signal_handlers_disconnect_by_func(
        panel->mnb, G_CALLBACK(multi_notebook_tabs_reordered), panel);
    g_signal_handlers_disconnect_by_func(
        panel->mnb, G_CALLBACK(multi_notebook_tab_switched), panel);

    G_OBJECT_CLASS(bedit_documents_panel_parent_class)->finalize(object);
}

static void bedit_documents_panel_dispose(GObject *object) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(object);
...
static GtkWidget *create_placeholder_row(gint height) {
    ...
    gtk_style_context_add_class(
        context, "bedit-document-panel-placeholder-row");
...
static void panel_on_drag_begin(GtkWidget *widget, GdkDragContext *context) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(widget);
    GtkWidget *drag_document_row;
    ...
    name = gtk_label_get_label(
        GTK_LABEL(BEDIT_DOCUMENTS_DOCUMENT_ROW(drag_document_row)->label));
    ...
    gtk_widget_set_size_request(
        panel->dnd_window, allocation.width, allocation.height);
    gtk_window_set_screen(
        GTK_WINDOW(panel->dnd_window),
        gtk_widget_get_screen(drag_document_row));

    style_context = gtk_widget_get_style_context(panel->dnd_window);
    gtk_style_context_add_class(
        style_context, "bedit-document-panel-dragged-row");
    ...
    gtk_drag_set_icon_widget(
        context, panel->dnd_window, panel->drag_document_row_x,
        panel->drag_document_row_y);
}

static gboolean panel_on_drag_motion(
    ...
    if (target !=
        gdk_atom_intern_static_string("BEDIT_DOCUMENTS_DOCUMENT_ROW")) {
        gdk_drag_status(context, 0, time);
        return FALSE;
    }

    gtk_widget_translate_coordinates(
        widget, panel->listbox, x, y, &dest_x, &dest_y);

    generic_row = (BeditDocumentsGenericRow *)gtk_list_box_get_row_at_y(
        GTK_LIST_BOX(panel->listbox), dest_y);
    ...
    if (!panel->row_placeholder) {
        if (!generic_row) {
            ...
            panel->document_row_height =
                BEDIT_DOCUMENTS_PANEL(source_panel)->document_row_height;
        } else {
            GtkAllocation allocation;
        ...
        panel->row_placeholder =
            create_placeholder_row(panel->document_row_height);
    ...
    if (!generic_row) {
        ...
        GList *children =
            gtk_container_get_children(GTK_CONTAINER(panel->listbox));
        ...
        gtk_widget_translate_coordinates(
            widget, GTK_WIDGET(generic_row), x, y, &dest_x, &dest_y);
    ...
    if (source_panel == widget) {
        ...
        gint source_row_index = gtk_list_box_row_get_index(
            GTK_LIST_BOX_ROW(panel->drag_document_row));
        panel->row_source_row_offset =
            source_row_index < row_placeholder_index ? -1 : 0;
    }

    if (panel->row_placeholder_index != row_placeholder_index) {
        if (panel->row_placeholder_index != ROW_OUTSIDE_LISTBOX) {
            gtk_container_remove(
                GTK_CONTAINER(panel->listbox), panel->row_placeholder);
        ...
        panel->row_destination_index = panel->row_placeholder_index =
            row_placeholder_index;

        gtk_list_box_insert(
            GTK_LIST_BOX(panel->listbox), panel->row_placeholder,
...
static void panel_on_drag_leave(
    GtkWidget *widget, GdkDragContext *context, guint time) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(widget);
...
static gboolean panel_on_drag_drop(
    GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(widget);
    ...
    if (BEDIT_IS_DOCUMENTS_PANEL(source_widget)) {
        gtk_widget_show(
            BEDIT_DOCUMENTS_PANEL(source_widget)->drag_document_row);
    }
...
static void panel_on_drag_data_get(
    GtkWidget *widget, GdkDragContext *context, GtkSelectionData *data,
    ...
    if (target ==
        ...
        gtk_selection_data_set(
            data, target, 8, (void *)&panel->drag_document_row,
            sizeof(gpointer));
        return;
    }
    ...
    if (result != GDK_NONE) {
        ...
        tab = BEDIT_TAB(
            BEDIT_DOCUMENTS_DOCUMENT_ROW(panel->drag_document_row)->ref);
        doc = bedit_tab_get_document(tab);
        ...
        if (!bedit_document_is_untitled(doc)) {
            ...
            gtk_selection_data_set(
                data, target, 8, (guchar *)full_name, strlen(full_name));
            g_free(full_name);
        }
    }
...
static BeditNotebook *get_notebook_and_position_from_document_row(
    BeditDocumentsPanel *panel, gint row_index, gint *position) {
    GList *l;
    gint index = 0;
    ...
    *position = index;
    return BEDIT_NOTEBOOK(row->ref);
}

static void panel_on_drag_data_received(
    ...
    if (BEDIT_IS_DOCUMENTS_PANEL(source_widget)) {
        source_panel = BEDIT_DOCUMENTS_PANEL(source_widget);
    }
    ...
    if (source_panel &&
        gtk_selection_data_get_target(data) ==
            gdk_atom_intern_static_string("BEDIT_DOCUMENTS_DOCUMENT_ROW")) {
        gint source_index =
            gtk_list_box_row_get_index(GTK_LIST_BOX_ROW(*source_row));
        ...
        if (source_panel != panel ||
            ...
            BeditTab *tab =
                BEDIT_TAB(BEDIT_DOCUMENTS_DOCUMENT_ROW(*source_row)->ref);

            old_notebook = bedit_multi_notebook_get_notebook_for_tab(
                source_panel->mnb, tab);
            new_notebook = get_notebook_and_position_from_document_row(
                panel, panel->row_destination_index, &position);
            if (old_notebook == new_notebook) {
                gtk_widget_show(*source_row);

                gtk_notebook_reorder_child(
                    GTK_NOTEBOOK(new_notebook), GTK_WIDGET(tab),
                    position + panel->row_source_row_offset);
            } else {
                bedit_notebook_move_tab(
                    old_notebook, new_notebook, tab, position);
            }

            if (tab != bedit_multi_notebook_get_active_tab(panel->mnb)) {
                g_signal_handler_block(
                    panel->mnb, panel->tab_switched_handler_id);
...
static void panel_on_drag_end(GtkWidget *widget, GdkDragContext *context) {
    BeditDocumentsPanel *panel = BEDIT_DOCUMENTS_PANEL(widget);
...
static gboolean panel_on_drag_failed(
...
static gboolean panel_on_motion_notify(
    ...
    if (gtk_drag_check_threshold(
            widget, panel->drag_root_x, panel->drag_root_y, event->x_root,
            event->y_root)) {
        panel->is_on_drag = TRUE;

        gtk_drag_begin_with_coordinates(
            widget, panel->source_targets, GDK_ACTION_MOVE, GDK_BUTTON_PRIMARY,
            (GdkEvent *)event, -1, -1);
    }
...
static void bedit_documents_panel_class_init(BeditDocumentsPanelClass *klass) {
...
static void bedit_documents_panel_init(BeditDocumentsPanel *panel) {
    GtkWidget *sw;
    GtkStyleContext *context;
    ...
    gtk_orientable_set_orientation(
        GTK_ORIENTABLE(panel), GTK_ORIENTATION_VERTICAL);
    ...
    panel->adjustment =
        gtk_list_box_get_adjustment(GTK_LIST_BOX(panel->listbox));
    ...
    panel->selection_changed_handler_id = g_signal_connect(
        panel->listbox, "row-selected", G_CALLBACK(listbox_selection_changed),
        panel);
    panel->is_in_tab_switched = FALSE;
    ...
    panel->source_targets =
        gtk_target_list_new(panel_targets, G_N_ELEMENTS(panel_targets));
    gtk_target_list_add_text_targets(panel->source_targets, 0);

    gtk_drag_dest_set(
        GTK_WIDGET(panel), 0, panel_targets, G_N_ELEMENTS(panel_targets),
        GDK_ACTION_MOVE);
...
GtkWidget *bedit_documents_panel_new(BeditWindow *window) {
    ...
    return g_object_new(BEDIT_TYPE_DOCUMENTS_PANEL, "window", window, NULL);
}

static void row_on_close_button_clicked(
    GtkWidget *close_button, GtkWidget *row) {
    BeditDocumentsGenericRow *generic_row = (BeditDocumentsGenericRow *)row;
    BeditWindow *window = generic_row->panel->window;
    GtkWidget *ref;

    if (BEDIT_IS_DOCUMENTS_GROUP_ROW(row)) {
        ref = BEDIT_DOCUMENTS_GROUP_ROW(row)->ref;
        _bedit_cmd_file_close_notebook(window, BEDIT_NOTEBOOK(ref));
    } else if (BEDIT_IS_DOCUMENTS_DOCUMENT_ROW(row)) {
        ref = BEDIT_DOCUMENTS_DOCUMENT_ROW(row)->ref;
        _bedit_cmd_file_close_tab(BEDIT_TAB(ref), window);
    } else {
        g_assert_not_reached();
    }
}

static gboolean row_on_button_pressed(
    GtkWidget *row_event_box, GdkEventButton *event, GtkWidget *row) {
    if (gdk_event_get_event_type((GdkEvent *)event) == GDK_BUTTON_PRESS &&
        BEDIT_IS_DOCUMENTS_DOCUMENT_ROW(row)) {
        BeditDocumentsDocumentRow *document_row =
            BEDIT_DOCUMENTS_DOCUMENT_ROW(row);
        ...
        if (gdk_event_triggers_context_menu((GdkEvent *)event)) {
            ...
            BeditTab *tab = BEDIT_TAB(document_row->ref);
...
static void document_row_create_header(GtkWidget *row) {
    BeditDocumentsDocumentRow *document_row = BEDIT_DOCUMENTS_DOCUMENT_ROW(row);
    GtkWidget *image_box;
    gint width, height;
    ...
    gtk_box_pack_start(GTK_BOX(document_row->box), image_box, FALSE, FALSE, 0);

    /* Set the header on front of all other widget in the row */
    gtk_box_reorder_child(GTK_BOX(document_row->box), image_box, 0);

    gtk_widget_show_all(image_box);
}

static GtkWidget *row_create(GtkWidget *row) {
    BeditDocumentsGenericRow *generic_row = (BeditDocumentsGenericRow *)row;
    GtkWidget *event_box;
    GtkStyleContext *context;
    GtkWidget *image;
    ...
    generic_row->close_button = GTK_WIDGET(g_object_new(
        GTK_TYPE_BUTTON, "relief", GTK_RELIEF_NONE, "focus-on-click", FALSE,
        NULL));
    ...
    gtk_box_pack_start(
        GTK_BOX(generic_row->box), generic_row->label, FALSE, FALSE, 0);
    gtk_box_pack_start(
        GTK_BOX(generic_row->box), generic_row->status_label, FALSE, FALSE, 0);

    gtk_box_pack_end(
        GTK_BOX(generic_row->box), generic_row->close_button, FALSE, FALSE, 0);

    g_signal_connect(
        event_box, "button-press-event", G_CALLBACK(row_on_button_pressed),
        row);
    g_signal_connect(
        generic_row->close_button, "clicked",
        G_CALLBACK(row_on_close_button_clicked), row);
...
static gboolean document_row_query_tooltip(
    GtkWidget *row, gint x, gint y, gboolean keyboard_tip,
    GtkTooltip *tooltip) {
    BeditDocumentsGenericRow *generic_row = (BeditDocumentsGenericRow *)row;
    gchar *markup;
    ...
    markup = _bedit_tab_get_tooltip(BEDIT_TAB(generic_row->ref));
...
/* Bedit Document Row */
static void bedit_documents_document_row_class_init(
    BeditDocumentsDocumentRowClass *klass) {}

static void bedit_documents_document_row_init(BeditDocumentsDocumentRow *row) {
...
/* Bedit Group Row */
static void bedit_documents_group_row_class_init(
    BeditDocumentsGroupRowClass *klass) {}

static void bedit_documents_group_row_init(BeditDocumentsGroupRow *row) {
    GtkWidget *row_widget;
    GtkStyleContext *context;
...
static GtkWidget *bedit_documents_document_row_new(
    ...
    g_return_val_if_fail(BEDIT_IS_DOCUMENTS_PANEL(panel), NULL);
    g_return_val_if_fail(BEDIT_IS_TAB(tab), NULL);

    bedit_debug(DEBUG_PANEL);

    row = g_object_new(BEDIT_TYPE_DOCUMENTS_DOCUMENT_ROW, NULL);
    ...
    g_signal_connect(
        ...
        G_CALLBACK(document_row_sync_tab_name_and_icon), row);
    g_signal_connect(
        row->ref, "notify::state",
        G_CALLBACK(document_row_sync_tab_name_and_icon), row);
    g_signal_connect(
        row, "query-tooltip", G_CALLBACK(document_row_query_tooltip), NULL);

    document_row_sync_tab_name_and_icon(
        BEDIT_TAB(row->ref), NULL, GTK_WIDGET(row));
...
static GtkWidget *bedit_documents_group_row_new(
    ...
    g_return_val_if_fail(BEDIT_IS_DOCUMENTS_PANEL(panel), NULL);
    g_return_val_if_fail(BEDIT_IS_NOTEBOOK(notebook), NULL);

    bedit_debug(DEBUG_PANEL);

    row = g_object_new(BEDIT_TYPE_DOCUMENTS_GROUP_ROW, NULL);
